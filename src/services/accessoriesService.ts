...
import type { AccessoryItem, BaseItem } from '../types/store';

type PbAccessoryRecord = {
  id: string;
  name: string;
  seller_name: string;
  game_format: string;
...
const mapPbToAccessory = (record: PbAccessoryRecord): AccessoryItem => ({
  id: record.id,
  name: record.name,
...
const mapAccessoryToPb = (item: Omit<BaseItem, 'id'>): Omit<PbAccessoryRecord, 'id'> => ({
  name: item.name,
...
export const accessoriesService = {
  async listAll(): Promise<AccessoryItem[]> {
    const pageSize = 200;
    let page = 1;
    const all: AccessoryItem[] = [];

    while (true) {
      const result = await pb.collection(COLLECTION).getList<PbAccessoryRecord>(page, pageSize, {
        sort: '-created'
      });
      all.push(...result.items.map(mapPbToAccessory));
  ...
  async create(item: Omit<BaseItem, 'id'>): Promise<AccessoryItem> {
    const payload = mapAccessoryToPb(item);
    const created = await pb.collection(COLLECTION).create<PbAccessoryRecord>(payload);
    return mapPbToAccessory(created);
  },

  async update(id: string, item: Partial<Omit<BaseItem, 'id'>>): Promise<AccessoryItem> {
    const payload: Partial<Omit<PbAccessoryRecord, 'id'>> = {};
    ...
    const updated = await pb.collection(COLLECTION).update<PbAccessoryRecord>(id, payload);
    return mapPbToAccessory(updated);
  },
  ...
  async createMany(items: Array<Omit<BaseItem, 'id'>>): Promise<AccessoryItem[]> {
